...
// ✅ GET all employee attendance WITH routine details
router.get('/with-routine', async (req, res) => {
  try {
    const result = await db.query(
      `SELECT ea.*, 
              dr.drdayofweek, dr.drslot, dr.drsubjid, dr.drcourseid, dr.drclassroomid, 
         ...
         LEFT JOIN public.college_daily_routine dr
           ON ea.attcourseid = dr.drcourseid
          AND ea.attsubjectid = dr.drsubjid
          AND ea.attclassid = dr.drclassroomid
         ORDER BY ea.attts_in DESC`
    );
    res.status(200).json(result.rows);
  } catch (err) {
    console.error('Error fetching employee attendance with routine:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
// ✅ GET single employee attendance WITH routine details
router.get('/:attid/with-routine', async (req, res) => {
  const { attid } = req.params;
  try {
    const result = await db.query(
      `SELECT ea.*, 
    ...
    console.error('Error fetching attendance with routine:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
});
